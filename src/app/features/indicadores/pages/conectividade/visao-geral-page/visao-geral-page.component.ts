...
export class VisaoGeralPageComponent implements OnInit {
  ...
  protected readonly legendaItems: LegendaItemModel[] = LEGENDA_CONECTIVIDADE;
  ...
  protected buildPopupHtml = (ponto: MapaPontoBaseModel): string => {
    ...
    const classificacao = String(
      escola.classificacao ?? escola['classificacao_conectividade'] ?? 'Inexistente',
    );
  ...
  private rotaDetalhe(coEntidade: number): string {
    ...
    return `/indicadores/conectividade/escolas/${coEntidade}${ano}`;
  }

  protected onDetalheSolicitado(ponto: MapaPontoBaseModel): void {
    this.router.navigate(['/indicadores/conectividade/escolas', ponto.co_entidade], {
